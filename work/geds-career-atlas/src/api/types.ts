...
export type PublicPerson={person_id:string;display_name:string;observed_title:string;observed_classifications:string[];org_id:string;organization_name:string;snapshot_id:string;snapshot_as_of:string;source_url:string}
export type PeoplePage={items:PublicPerson[];total:number;limit:number;offset:number;available_classifications:string[];snapshot_id:string;quality_status:string;etag:string}
export type PeopleQuery={q?:string;classification?:string;sort?:"name"|"title";limit?:number;offset?:number}
