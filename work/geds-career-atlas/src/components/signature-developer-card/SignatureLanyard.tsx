...
import type { SignatureDeveloperCardPointer } from "./SignatureDeveloperCard"
import defaultCardModelSrc from "./assets/lanyard-card.glb"
import defaultLanyardTextureSrc from "./assets/lanyard.png"
import "./SignatureLanyard.css"
...
export type SignatureLanyardRenderCardProps = {
  interactive?: boolean
...
export type SignatureLanyardProps = {
  /** Rendered as DOM so consumers keep their own link and card semantics. */
  renderCard: (props: SignatureLanyardRenderCardProps) => ReactNode
  position?: [number, number, number]
  gravity?: [number, number, number]
  fov?: number
  transparent?: boolean
  /** Optional image to decode before physics begins. */
  portraitSrc?: string
  /** Asset contract: GLTF must expose `clip`, `clamp`, and `metal`. */
  cardModelSrc?: string
...
type BandProps = {
  cardModelSrc: string
  renderCard: SignatureLanyardProps["renderCard"]
  maxSpeed?: number
  minSpeed?: number
  isMobile?: boolean
  lanyardImage?: string | null
...
function useImageReady(src: string | undefined, enabled: boolean) {
...
export function SignatureLanyard({
  renderCard,
  position = [0, 0, 30],
  gravity = [0, -40, 0],
  fov = 20,
  transparent = true,
  portraitSrc,
  cardModelSrc = defaultCardModelSrc,
  lanyardImage = null,
  lanyardWidth = 1,
}: SignatureLanyardProps) {
  const wrapperRef = useRef<HTMLDivElement>(null)
  const [isMobile, setIsMobile] = useState(() => typeof window !== "undefined" && window.innerWidth < 768)
  const [webGLAvailable] = useState(supportsWebGL)
  const reducedMotion = useReducedMotion()
  const photoReady = useImageReady(portraitSrc, Boolean(portraitSrc) && webGLAvailable && !reducedMotion)
  ...
  if (reducedMotion || !webGLAvailable) {
    return <div
      ref={wrapperRef}
      className="signature-lanyard signature-lanyard--static"
  ...
  return <div ref={wrapperRef} className="signature-lanyard" data-camera-distance={position[2]} data-render-mode="physics" data-photo-ready="true">
    <Canvas
      camera={{ position, fov }}
...
function Band({
  ...
  const texture = useTexture(lanyardImage || defaultLanyardTextureSrc)
  ...
  const pointerToWorld = (pointer: SignatureDeveloperCardPointer, planeZ: number) => {
    ...
    <group position={[originX, DESKTOP_ORIGIN_Y, 0]}>
      ...
      <RigidBody position={[2, 0, 0]} ref={card} {...segmentProps} type={dragged ? "kinematicPosition" : "dynamic"}>
        ...
        <Html
          ...
          pointerEvents="auto"
          wrapperClass="signature-lanyard-profile-host"
          className="signature-lanyard-profile-content"
          zIndexRange={[40, 1]}
        >
          {renderCard({
...
export default SignatureLanyard
