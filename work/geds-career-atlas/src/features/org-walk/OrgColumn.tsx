...
export function OrgColumn({label,items,columnIndex,expandedId,onDrill,onProfile,onBack}:Props){
  ...
  return <BorderGlow as="section" className="org-column-shell" aria-label={label} glowIntensity={0.54} fillOpacity={0.055}><header><span>{label}</span><strong>{formatNumber(items.length)}</strong></header><div ref={scrollRef} className="org-root-list" role="list" aria-label={label} onKeyDown={event=>typeahead(event.key)}>
