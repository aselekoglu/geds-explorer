...
export function OrgColumn({label,items,columnIndex,expandedId,onDrill,onProfile,onBack}:Props){
  ...
  return <div ref={scrollRef} className="org-root-list" role="list" aria-label={label} onKeyDown={event=>typeahead(event.key)}>
    <div className="org-virtual-space" role="presentation" style={{height:virtualizer.getTotalSize()||Math.min(items.length,60)*62}}>
