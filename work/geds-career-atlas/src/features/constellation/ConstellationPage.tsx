...
type SliceClient = { constellationSlice: (rootId?: string, signal?: AbortSignal) => Promise<ConstellationSlice>; constellation?: (query: string, optionsOrSignal?: { department?: string; entity_kind?: "organization" | "person" } | AbortSignal, signal?: AbortSignal) => Promise<SearchResult> }
...
export function ConstellationPage({ client, query = "", focus, onProfile, scope={department:""},rootOrgId }: { client: SliceClient; query?: string; focus?: string; onProfile?: (orgId: string) => void;scope?:DiscoverScope;rootOrgId?:string }) {
  ...
  const [queryState,setQueryState]=useState<"idle"|"loading"|"empty"|"error">("idle")
  ...
  useEffect(()=>{const controller=new AbortController();const value=query.trim();setMatches(null);if(!value||!client.constellation){setQueryState("idle");return()=>controller.abort()};setQueryState("loading");client.constellation(value,{department:scope.department||undefined,entity_kind:"organization"},controller.signal).then(result=>{setMatches(result);setQueryState(result.items.length?"idle":"empty")}).catch(reason=>{if(reason.name!=="AbortError"){setMatches(null);setQueryState("error")}});return()=>controller.abort()},[client,query,scope.department])

  const interestNodes=useMemo(()=>{const grouped=new Map<string,ConstellationNode&{department_name:string}>();for(const item of matches?.items??[]){if(!item.org_id)continue;if(scope.department&&item.department_name!==scope.department)continue;const current=grouped.get(item.org_id);if(current){current.value=(current.value??0)+Math.max(1,item.score);current.match_count=(current.match_count??0)+1}else grouped.set(item.org_id,{id:item.org_id,name:item.organization_name,value:Math.max(1,item.score),match_count:1,department_name:item.department_name??"",vacancy_count:item.vacancy_signal?1:0})}return [...grouped.values()]},[scope,matches])
  ...
  if(!view.slice)return <section className="constellation-page"><h2>{t("constellation.title")}</h2><p role="status">{t("constellation.unavailable")}</p></section>
  return <section className="constellation-page" id="constellation">
    <header className="constellation-heading">{view.history.length>1&&<button type="button" className="constellation-back" onClick={goBack}>{t("common.back")}</button>}<div><h2>{query.trim()?t("constellation.queryTitle",{query:query.trim()}):t("constellation.title")}</h2><p>{query.trim()?t("constellation.queryIntro",{count:formatNumber(interestNodes.length)}):t("constellation.intro")}</p></div></header>
    ...
    {hasQuery&&queryState==="loading"?<p role="status" className="constellation-loading">{t("orgWalk.searching")}</p>:hasQuery&&queryState==="error"?<p role="alert" className="constellation-error">{t("constellation.searchError")}</p>:hasQuery&&(queryState==="empty"||!interestNodes.length)?<p role="status" className="constellation-empty">{t("discover.noMatch")}</p>:<BorderGlow className="constellation-stage" data-testid="constellation-stage" animated={false} borderRadius={4} backgroundColor="var(--canvas)" onClick={event=>{const target=event.target as Element;if(!target.closest(".constellation-node, .constellation-toolbar, .constellation-info-panel, [role='option']"))setInspection(null)}}><DotField bulgeStrength={18} dotSpacing={22} cursorRadius={280} waveAmplitude={0.35} glowRadius={80}/><ConstellationBoundary nodes={visualNodes} label={t("constellation.map")} focus={activeFocus} onSelect={id=>{const node=visualNodes.find(item=>item.id===id);if(node)selectNode(node)}} onDrill={id=>{const node=visualNodes.find(item=>item.id===id);if(node)drill(node)}}><Constellation nodes={visualNodes} focus={activeFocus} onDrill={drill} onSelect={selectNode} topLevel={!rootId} queryMode={hasQuery}/></ConstellationBoundary>{inspection&&<ConstellationInfoPanel node={inspection} onClose={()=>setInspection(null)} onProfile={onProfile}/>}</BorderGlow>}
