...
export function ConstellationPage({ client, query = "", focus, onFocus, scope={department:""},rootOrgId }: { client: SliceClient; query?: string; focus?: string; onFocus?: (orgId: string) => void;scope?:DiscoverScope;rootOrgId?:string }) {
  const [slice, setSlice] = useState<ConstellationSlice | null>(null)
  const [matches, setMatches] = useState<SearchResult | null>(null)
  const [rootId, setRootId] = useState<string | undefined>(rootOrgId)
  ...
  function reset() { setRootId(rootOrgId); setLocalFocus(undefined) }
  ...
  return <section className="constellation-page" id="constellation">
    <header className="constellation-heading"><div><h1>{query.trim() ? t("constellation.queryTitle", { query: query.trim() }) : t("constellation.title")}</h1><p>{query.trim() ? t("constellation.queryIntro", { count: formatNumber(interestNodes.length) }) : t("constellation.intro")}</p></div>{rootId&&rootId!==rootOrgId&&<button type="button" onClick={reset}>{t("constellation.all")}</button>}</header>
    <div className="constellation-stage"><ConstellationBoundary nodes={visualNodes} label={t("constellation.map")} focus={activeFocus} onFocus={select}><Constellation nodes={visualNodes} focus={activeFocus} onFocus={select} topLevel={!rootId} /></ConstellationBoundary>{(selected || selectedMatch) && <aside className="constellation-evidence" aria-live="polite"><h2>{selected?.name ?? selectedMatch?.organization_name}</h2><p><strong>{t("common.matchedBecause")}</strong> {selectedMatch?.evidence[0]?.source_text ?? t("constellation.hierarchyEvidence")}</p>{selected && <dl><div><dt>{t("common.peopleIndexed")}</dt><dd>{formatNumber(selected.descendant_people_count)}</dd></div><div><dt>{t("common.childTeams")}</dt><dd>{formatNumber(selected.child_count)}</dd></div></dl>}{selected?.child_count ? <button type="button" onClick={() => setRootId(selected.org_id)}>{t("constellation.explore")}</button> : null}</aside>}</div>
