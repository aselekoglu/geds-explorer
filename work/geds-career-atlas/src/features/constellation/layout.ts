...
function compareByValue(a:PackInput,b:PackInput){return (b.value??1)-(a.value??1)||a.id.localeCompare(b.id)}
function packValue(node:PackInput){return Math.max(1,node.value??1)}

export function buildPackLayout(nodes:PackInput[],width=800,height=520):PositionedNode[]{
  if(nodes.length===0)return []
  const sorted=[...nodes].sort(compareByValue)
  const root=hierarchy<PackRoot | PackInput>({children:sorted}).sum(data=>packValue(data as PackInput)).sort((a,b)=>(b.value??0)-(a.value??0)||String((a.data as PackInput).id??"").localeCompare(String((b.data as PackInput).id??"")))
  const packed=pack<PackRoot | PackInput>().size([width,height]).padding(3)(root)
