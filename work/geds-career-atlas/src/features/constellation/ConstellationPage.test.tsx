...
it("loads the bounded slice and synchronizes selected focus", async () => {
  const onFocus=vi.fn()
  const client={constellationSlice:async()=>({nodes:[{org_id:"statcan",name:"Statistics Canada",depth:0,child_count:8,direct_people_count:200,descendant_people_count:2400}],limit:2000,truncated:false,snapshot_id:"snapshot",etag:"etag"})}
...
it("reloads the hierarchy slice when institution root changes",async()=>{
  const constellationSlice=vi.fn().mockResolvedValue({nodes:[{org_id:"team",name:"Team",depth:1,child_count:0,direct_people_count:2,descendant_people_count:2}],limit:2000,truncated:false,snapshot_id:"snapshot",etag:"etag"})
