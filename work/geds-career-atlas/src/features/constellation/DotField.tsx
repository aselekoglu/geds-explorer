...
// The field only animates while the pointer is actively moving.  A 60fps cap
// keeps motion fluid on normal displays without spending a frame budget while
// the map is idle.
...
export function DotField({bulgeStrength,dotSpacing,cursorRadius,waveAmplitude,glowRadius}:DotFieldProps){
  ...
  useEffect(()=>{
    ...
    let visible=true, hidden=document.hidden, disposed=false, staticPainted=false, idlePainted=false, dirty=true, last=0, raf:number|undefined
    let width=1,height=1,dpr=1,color="#3b7c80"
    const updateMetrics=()=>{const rect=stage.getBoundingClientRect();width=Math.max(1,rect.width);height=Math.max(1,rect.height);dpr=Math.min(window.devicePixelRatio||1,2);color=getComputedStyle(stage).getPropertyValue("--accent").trim()||"#3b7c80";const pixelWidth=Math.round(width*dpr),pixelHeight=Math.round(height*dpr);if(canvas.width!==pixelWidth||canvas.height!==pixelHeight){canvas.width=pixelWidth;canvas.height=pixelHeight;canvas.style.width=`${width}px`;canvas.style.height=`${height}px`};staticPainted=false;idlePainted=false;dirty=true}
    ...
    const draw=(now:number)=>{raf=undefined
      ...
      if(!reduced&&now-last<frameInterval){schedule();return};last=now
      dirty=false
      ...
      context.fill();context.globalAlpha=1;staticPainted=reduced;idlePainted=!current.active
    }
    const onMove=(event:PointerEvent)=>{const rect=stage.getBoundingClientRect();pointer.current={x:event.clientX-rect.left,y:event.clientY-rect.top,active:true};idlePainted=false;dirty=true;schedule()}
    const onLeave=()=>{pointer.current.active=false;idlePainted=false;dirty=true;schedule()}
    const onVisibility=()=>{hidden=document.hidden;if(!hidden){staticPainted=false;idlePainted=false;dirty=true;schedule()}}
    const intersection=new IntersectionObserver(([entry])=>{visible=entry.isIntersecting;if(visible){staticPainted=false;idlePainted=false;dirty=true;schedule()}},{threshold:.01})
  ...
  return <div className="dot-field" data-testid="dot-field" data-bulge-strength={bulgeStrength} data-dot-spacing={dotSpacing} data-cursor-radius={cursorRadius} data-wave-amplitude={waveAmplitude} data-glow-radius={glowRadius} data-active-frame-rate="60"><canvas ref={canvasRef} aria-hidden="true" /></div>
}
