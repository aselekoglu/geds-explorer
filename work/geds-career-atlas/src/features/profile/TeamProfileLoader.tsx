...
export function TeamProfileLoader({ orgId, client,onRoleQuery }: { orgId: string; client: ProfileClient;onRoleQuery?:(title:string)=>void }) {
  const { t } = useLanguage()
  ...
  return <TeamProfile name={data.profile.name} roles={data.roles.items.map(item => item.title)} profile={data.profile} relatedTeams={data.children.map(item=>({org_id:item.org_id,name:item.name}))} peopleClient={client.people?{people:client.people.bind(client)}:undefined} onRoleQuery={onRoleQuery} />
}
