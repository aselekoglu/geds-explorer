...
export function TeamProfile({ name, roles, roleItems = [], profile, relatedTeams = [], peopleClient }: { name: string; roles: string[]; roleItems?: SearchResult["items"]; profile?: ProfileFacts; relatedTeams?: RelatedTeam[]; peopleClient?:PeopleClient }) {
  ...
  return <section aria-label={`${name} team profile`}>
    ...
    {profile?.child_count===0&&peopleClient&&<PeopleInTeam orgId={profile.org_id} client={peopleClient}/>}<h3>{t("profile.rolesTitle")}</h3><p>{t("profile.rolesIntro")}</p><ul>{roles.map((role,index) => <li key={`${role}:${index}`}>{role}</li>)}</ul>
