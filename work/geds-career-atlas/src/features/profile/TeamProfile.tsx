...
export function TeamProfile({ name, roles, profile, relatedTeams = [], peopleClient,onRoleQuery }: { name: string; roles: string[]; profile?: ProfileFacts; relatedTeams?: RelatedTeam[]; peopleClient?:PeopleClient;onRoleQuery?:(title:string)=>void }) {
  ...
  return <section aria-label={`${name} team profile`}>
    ...
    {profile?.child_count===0&&peopleClient&&<PeopleInTeam orgId={profile.org_id} client={peopleClient}/>}<h3>{t("profile.rolesTitle")}</h3><p>{t("profile.rolesIntro")}</p><GroupedRoles titles={roles} onRoleQuery={onRoleQuery}/>
